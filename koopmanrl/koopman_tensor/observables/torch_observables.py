...
class gaussians(object):
    ...
    def diff(self, x):
        ...
        for i in range(n):  # for all Gaussians
            for j in range(d):  # for all dimensions
                y[i, j, :] = (
                    -2 / (2 * self.sigma**2) * (x[j, :] - c[j, i]) * torch.exp(-1 / (2 * self.sigma**2) * D[i, :])
                )
        ...
        for i in range(n):  # for all Gaussians
            for j1 in range(d):  # for all dimensions
                for j2 in range(d):  # for all dimensions
                    if j1 == j2:
                        y[i, j1, j2, :] = (
                            -2 / (2 * self.sigma**2) + 4 / (4 * self.sigma**4) * (x[j1, :] - c[j1, i]) ** 2
                        ) * torch.exp(-1 / (2 * self.sigma**2) * D[i, :])
                    else:
                        y[i, j1, j2, :] = (
                            4 / (4 * self.sigma**4) * (x[j1, :] - c[j1, i]) * (x[j2, :] - c[j2, i])
                        ) * torch.exp(-1 / (2 * self.sigma**2) * D[i, :])
        return y
